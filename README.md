# Aurora Analytics — Anugal CIAM demo application
...
[Meridian Docs](https://github.com/) (update this link once that repo is
pushed).
...
application — such as [Meridian Docs](https://github.com/), its sibling —
registered as a separate client under the **same** Anugal customer:

1. Sign in here. The page shows the claims from the ID token and confirms the
   signature verified against the Anugal JWKS.
2. Open the sibling application. It should complete with no second prompt —
   that is single sign-on. The session lives in Anugal, not in either page.
